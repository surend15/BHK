...
export default function Cottage ({navigation}){
 ...
 useEffect(()=>{
      fetch('http://192.168.152.186:8000/cot',{
  method: 'post',
  headers: {
  ...
  return (
       <View style={{flex:1,backgroundColor:"white"}}>
        <View style={{width:"100%",height: "12%",
                           ...
                           alignItems:"center"}}>
                             <Text style={{color:"white",fontSize:25,justifyContent:"center",fontWeight:"bold",marginTop:"10%"}}>COTTAGE HOUSE</Text>
                             <Add name='export' size={32} style={{marginLeft:"80%"}} onPress={()=>{setsample(true);setwhat('cot')}}/>
        </View> 
        <FlatList
        data={data}
        renderItem={(item)=>(
