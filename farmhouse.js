...
export default function Farm ({navigation}){
 ...
 useEffect(()=>{
      fetch('http://192.168.230.186:8000/far',{
  method: 'post',
  headers: {
  ...
  return (
       <View style={{flex:1,backgroundColor:"white"}}>
        <View style={{width:"100%",height: "12%",
                           ...
                           alignItems:"center"}}>
                             <Text style={{color:"white",fontSize:25,justifyContent:"center",fontWeight:"bold",marginTop:"10%"}}>FARM HOUSE</Text>
                             <Add name='export' size={32} style={{marginLeft:"80%"}} onPress={()=>{setsample(true);setwhat('far')}}/>
        </View> 
        <FlatList
        data={data}
        renderItem={(item)=>(
